...
unsigned int score(unsigned char user_input,tree * tree_node);
unsigned int tree_navigator(memory_struct  current_pos);
unsigned int referee(omok_space * omok);
...
unsigned int score(unsigned char user_input,tree * tree_node)// 실험자가 둔 수에 대한 예측된 수의 점수 계산 
{
...
unsigned int referee(omok_space * omok)// 경기 심판 오목 탐지기능 
{

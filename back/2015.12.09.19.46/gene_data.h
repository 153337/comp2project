...
unsigned char score(unsigned char user_input,tree * tree_node,omok_space * omok,unsigned int code);
unsigned char referee(omok_space * omok,unsigned char pos);
...
unsigned char vote_gene(gene_header** read_address,unsigned char *pos_array,omok_space * omok,unsigned int turn);

unsigned int exception_handler(unsigned int code,void * data);//



unsigned char score(unsigned char user_input,tree * tree_node,omok_space * omok,unsigned int code)// 실험자가 둔 수에 대한 예측된 수의 점수 계산 
{
	
	unsigned int score_high[81];
	unsigned char pos[81],x2,y2;
	unsigned int counter,counter2 ,header=0;
	
	
	for(counter =0;counter<8;counter++)
	printf("%d위치 %d 값\n",tree_node->next[counter]->pos,tree_node->next[counter]->value );
	...
	{	
		for(counter =0;counter<8;counter++)
		{
				y2= tree_node->next[counter2]->next[counter]->pos << 4;
				y2= y2 >> 4;	
				x2= tree_node->next[counter2]->next[counter]->pos >> 4;
			
			
			printf(" 서브위치 %d %d  값 %d \n",x2,y2,tree_node->next[counter2]->next[counter]->value );
			if(tree_node->next[counter2]->next[counter]->value == 0)// 비어있는 공간 
			{
				pos[header] = referee(omok,tree_node->next[counter2]->next[counter]->pos);
				header++;
			}
...
unsigned char referee(omok_space * omok,unsigned char pos)// 경기 심판 오목 탐지기능 
{
...
unsigned char vote_gene(gene_header** read_address,unsigned char *pos_array,omok_space * omok,unsigned int turn)// 염색체 선발 
{
	
	unsigned int counter=0,counter2=0,grade=0;
	...
	for(counter =0;counter < population_size; counter++)
	{
		...
		for(counter2=0;counter2<turn;counter2++)//해당되는 턴 까지 점프 
		{
			start = start->next_node;
		}
		
		if(start->temper == pos_array[grade])
		{
			
		}// 뽑힌 염색체  해당 정보 기록  그리고 수를 둔다. 
	...
	else if( x == 0 || y == 0 || x == 15 || y == 15 )
	{
		...
		if(mode == 1)
		{
			...
			for(counter2 =0 ;counter2<8; counter2++ )
			{
				...
				{
					tree_node->next[counter2]->value = 0;
					tree_node->next[counter2]->pos = tree_node->pos +x_in_de_crease*pattern[counter2][0] +y_in_de_crease*pattern[counter2][1];
				}
				else if( omok->space[x+ pattern[counter2][0]][y+pattern[counter2][1]] == 1)
				{
					tree_node->next[counter2]->value = 1;
...
unsigned int exception_handler(unsigned int code,void * data)
{
	...
	else if(code == 2)// 가장자리 초과 (엉뚱한 값 참조시) 
	{
...
unsigned int learn_fight(gene_header** read_address) // 프로그램을 학습 시킨다. 
{
	...
	unsigned int fun_return;
	unsigned int keyboard_key;
	unsigned int counter=0,counter2 =0;
	unsigned char x,y,pos,x2,y2;
	...
	while(1)
	{
		...
		if(keyboard_key == 13 && fight_place.space[x][y] == 0)
		{
			...
			printf("입력\n");
			fun_return = fitness_selector(0,pos,&fight_place,make_tree,0);
			...
			score(pos,make_tree,&fight_place,fun_return); // 적합도를 판정하고 돌을 두는 함수 
			
			free(make_tree); // 트리의 초기화 
			
			x = x >> 4;// x 좌표를 원래대로 바꾼다. 
		}
		else if(fight_place.space[x][y] != 0)
		{
			printf("이미 돌이 놓인곳입니다.\n");
		}
		...
		for(counter =0;counter < space_height ;counter ++)
		{
			printf("\n");
			for(counter2=0 ;counter2 < space_width ;counter2 ++ )
			{
				if(x== counter2 && y == counter)
				{
					printf("x ");
				}
				else
